...
DB_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "finextract.db")
...
def delete_document(doc_id):
    conn = get_db_connection()
    cursor = conn.cursor()
    # Fetch filepath to delete it locally
...
def save_extracted_kpi(doc_id, kpi_name, value_raw, value_numeric, year, page, source_text, confidence, is_custom=0):
    conn = get_db_connection()
    cursor = conn.cursor()
    # Check if duplicate entry already exists to avoid clutter
    cursor.execute("""
